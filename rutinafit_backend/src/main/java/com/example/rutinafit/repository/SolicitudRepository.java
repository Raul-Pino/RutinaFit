...
public interface SolicitudRepository extends JpaRepository<Solicitud, Long> {
    ...
    // List<Solicitud> findRecientes(Long destId, String estado, LocalDateTime
    // fechaLimite);

    // @Transactional // Operación para borrar solicitudes pendientes después de 30
    // días
    ...
    @Transactional
    @Modifying(clearAutomatically = true)
    @Query("DELETE FROM Solicitud s WHERE " +
            "((s.remitente.id = :u1 AND s.destinatario.id = :u2) OR " +
            " (s.remitente.id = :u2 AND s.destinatario.id = :u1))")
    void borrarSolicitud(@Param("u1") Long u1, @Param("u2") Long u2);
    ...
    @Transactional
    @Query("SELECT s FROM Solicitud s WHERE s.tipo = :tipo AND " +
            "((s.remitente.id = :u1 AND s.destinatario.id = :u2) OR " +
            " (s.remitente.id = :u2 AND s.destinatario.id = :u1))")
    java.util.Optional<Solicitud> findByUsuariosYTipo(@Param("u1") Long u1, @Param("u2") Long u2,
            @Param("tipo") TipoSolicitud tipo);

    @Transactional
    @Modifying
    @Query("DELETE FROM Solicitud s WHERE s.remitente.id = :remId AND s.tipo = :tipo AND s.estado = :estado")
    void deleteByIdRemitenteTipoandEstado(@Param("remId") Long remId, @Param("tipo") TipoSolicitud tipo,
