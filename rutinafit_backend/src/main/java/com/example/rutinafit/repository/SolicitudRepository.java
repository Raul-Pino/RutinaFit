...
public interface SolicitudRepository extends JpaRepository<Solicitud, Long> {
    ...
    @Query("SELECT s FROM Solicitud s WHERE s.tipo = :tipo AND " +
        "((s.remitente.id = :u1 AND s.destinatario.id = :u2) OR " +
        " (s.remitente.id = :u2 AND s.destinatario.id = :u1))")
    java.util.Optional<Solicitud> findByUsuariosYTipo(@Param("u1") Long u1, @Param("u2") Long u2, @Param("tipo") TipoSolicitud tipo);

    @Transactional
    @Modifying
    @Query("DELETE FROM Solicitud s WHERE s.remitente.id = :remId AND s.tipo = :tipo AND s.estado = :estado")
    void deleteByIdRemitenteTipoandEstado(@Param("remId") Long remId, @Param("tipo") TipoSolicitud tipo, @Param("estado") EstadoSolicitud estado);
